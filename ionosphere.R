...
ionosphere <- read.csv("C:/Users/Ray Chen/Desktop/data/ionosphere.data",header = FALSE)
ionosphere.name <- colnames(ionosphere)
ionosphere.name[length(ionosphere.name)] <- "ionospheretype"
colnames(ionosphere) <- ionosphere.name
ionosphere$ionospheretype <- as.factor(ionosphere$ionospheretype)

ionosphere <- ionosphere[, -2]
for (i in 1:(ncol(ionosphere)-1)) {
  ionosphere[,i] <- as.numeric(ionosphere[,i])
}
...
# PCA
pca_result <- prcomp(ionosphere[, 1:(ncol(ionosphere)-1)], center = TRUE, scale. = FALSE)
summary(pca_result)
pca_features <- as.data.frame(pca_result$x[,1:22])
...
max_it <- 10
set.seed(123)
folds <- createFolds(ionosphere$ionospheretype, k = max_it, list = TRUE, returnTrain = TRUE)
...
for (m in 1:max_it) {
  ...
  #folds <- createFolds(ionosphere$ionospheretype, k = max_it, list = TRUE, returnTrain = TRUE)
  index <- folds[[m]]
  X_train <- ionosphere[index, 1:(ncol(ionosphere)-1)]
  X_test <- ionosphere[-index, 1:(ncol(ionosphere)-1)]
  Y_train <- ionosphere[index, (ncol(ionosphere))]
  Y_test <- ionosphere[-index, (ncol(ionosphere))]
...
output <- matrix(0,6,ncol(accuracy_matrix))
colnames(output) <- colnames(accuracy_matrix)
for (i in 1:(ncol(accuracy_matrix))) {
  output[,i] <- sort(accuracy_matrix[, i],decreasing = TRUE)[1:6]
}
print(output)
