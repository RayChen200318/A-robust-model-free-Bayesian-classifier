...
redwinequality <- read.csv2("C:/Users/Ray Chen/Desktop/winequality-red.csv")
redwinequality$quality <- as.factor(redwinequality$quality)
for (i in 1:(ncol(redwinequality)-1)) {
  redwinequality[,i] <- as.numeric(redwinequality[,i])
  redwinequality[,i] <- scale(redwinequality[,i])
}
...
# PCA
pca_result <- prcomp(redwinequality[, 1:(ncol(redwinequality)-1)], center = TRUE, scale. = TRUE)
summary(pca_result)
...
max_it <- 10
set.seed(123)
folds <- createFolds(redwinequality$quality, k = max_it, list = TRUE, returnTrain = TRUE)
...
for (m in 1:max_it) {
  pb$tick()
  index <- folds[[m]]
  X_train <- redwinequality[index, 1:(ncol(redwinequality)-1)]
  X_test <- redwinequality[-index, 1:(ncol(redwinequality)-1)]
  Y_train <- redwinequality[index, (ncol(redwinequality))]
  Y_test <- redwinequality[-index, (ncol(redwinequality))]
