...
segment1 <- read.csv("C:/Users/Ray Chen/Desktop/data/segmentation.data",header = FALSE)
segment2 <- read.csv("C:/Users/Ray Chen/Desktop/data/segmentation.test",header = FALSE)
colnames(segment1) <- segment1[3,]
segment1 <- segment1[-1:-3,]
colnames(segment2) <- segment2[3,]
segment2 <- segment2[-1:-3,]
segment <- rbind(segment1,segment2)

segmenttype <- segment[,1]
segment <- segment[,-1]
segment <- cbind(segment, segmenttype)
segment$segmenttype <- as.factor(segment$segmenttype)
for (i in 1:(ncol(segment)-1)) {
  segment[,i] <- as.numeric(segment[,i])
}
...
# PCA
pca_result <- prcomp(segment[, 1:(ncol(segment)-1)], center = TRUE, scale. = FALSE)
summary(pca_result)
...
max_it <- 10
set.seed(123)
folds <- createFolds(segment$segmenttype, k = max_it, list = TRUE, returnTrain = TRUE)
...
for (m in 1:max_it) {
  ...
  #folds <- createFolds(segment$segmenttype, k = max_it, list = TRUE, returnTrain = TRUE)
  index <- folds[[m]]
  X_train <- segment[index, 1:(ncol(segment)-1)]
  X_test <- segment[-index, 1:(ncol(segment)-1)]
  Y_train <- segment[index, (ncol(segment))]
  Y_test <- segment[-index, (ncol(segment))]
