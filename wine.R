...
wine <- read.csv(gzfile("C:/Users/Ray Chen/Desktop/data/wine.data"), header=FALSE)
winetype <- wine[,1]
wine <- wine[,-1]
wine <- cbind(wine, winetype)
...
# PCA
pca_result <- prcomp(wine[, 1:(ncol(wine)-1)], center = TRUE, scale. = FALSE)
summary(pca_result)
...
max_it <- 10
set.seed(123)
folds <- createFolds(wine$winetype, k = max_it, list = TRUE, returnTrain = TRUE)
