...
for (i in 1:(ncol(iris)-1)) {
  iris[,i] <- as.numeric(iris[,i])
}
iris$Species <- as.factor(iris$Species)
# Instance clone process
pca_result <- prcomp(iris[, 1:(ncol(iris)-1)], center = TRUE, scale. = TRUE)
summary(pca_result)
pca_features <- as.data.frame(pca_result$x[, 1:2])
...
max_it <- 10
set.seed(123)
folds <- createFolds(iris$Species, k = max_it, list = TRUE, returnTrain = TRUE)
...
m <- 1
for (m in 1:max_it) {
  #folds <- createFolds(iris$quality, k = max_it, list = TRUE, returnTrain = TRUE)
  index <- folds[[m]]
  X_train <- iris[index, 1:(ncol(iris)-1)]
  X_test <- iris[-index, 1:(ncol(iris)-1)]
  Y_train <- iris[index, (ncol(iris))]
  Y_test <- iris[-index, (ncol(iris))]
