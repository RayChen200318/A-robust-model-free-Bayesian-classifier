...
glass <- read.csv("C:/Users/Ray Chen/Desktop/data/glass.data",header = FALSE)
glass.name <- colnames(glass)
glass.name[length(glass.name)] <- "glasstype"
colnames(glass) <- glass.name

glass$glasstype <- as.factor(glass$glasstype)
for (i in 1:(ncol(glass)-1)) {
  glass[,i] <- as.numeric(glass[,i])
}
...
# PCA
pca_result <- prcomp(glass[, 1:(ncol(glass)-1)], center = TRUE, scale. = FALSE)
summary(pca_result)
pca_features <- as.data.frame(pca_result$x[,1:7])
...
max_it <- 10
set.seed(123)
folds <- createFolds(glass$glasstype, k = max_it, list = TRUE, returnTrain = TRUE)
...
for (m in 1:max_it) {
  ...
  #folds <- createFolds(glass$glasstype, k = max_it, list = TRUE, returnTrain = TRUE)
  index <- folds[[m]]
  X_train <- glass[index, 1:(ncol(glass)-1)]
  X_test <- glass[-index, 1:(ncol(glass)-1)]
  Y_train <- glass[index, (ncol(glass))]
  Y_test <- glass[-index, (ncol(glass))]
...
output <- matrix(0,9,ncol(accuracy_matrix))
colnames(output) <- colnames(accuracy_matrix)
for (i in 1:(ncol(accuracy_matrix))) {
  output[,i] <- sort(accuracy_matrix[, i],decreasing = TRUE)[1:9]
}
print(output)
