...
drybean <- read_xlsx("C:/Users/Ray Chen/Desktop/data/Dry_Bean_Dataset.xlsx")
drybean.name <- colnames(drybean)
drybean.name[length(drybean.name)] <- "drybeantype"
colnames(drybean) <- drybean.name
drybean <- as.data.frame(drybean)

drybean$drybeantype <- as.factor(drybean$drybeantype)
for (i in 1:(ncol(drybean)-1)) {
  drybean[,i] <- as.numeric(drybean[,i])
  drybean[,i] <- scale(drybean[,i])
}
...
# PCA
pca_result <- prcomp(drybean[, 1:(ncol(drybean)-1)], center = TRUE, scale. = FALSE)
summary(pca_result)
pca_features <- as.data.frame(pca_result$x[,1:6])
...
max_it <- 10
set.seed(123)
folds <- createFolds(drybean$drybeantype, k = max_it, list = TRUE, returnTrain = TRUE)
...
for (m in 1:max_it) {
  ...
  #folds <- createFolds(drybean$drybeantype, k = max_it, list = TRUE, returnTrain = TRUE)
  index <- folds[[m]]
  X_train <- drybean[index, 1:(ncol(drybean)-1)]
  X_test <- drybean[-index, 1:(ncol(drybean)-1)]
  Y_train <- drybean[index, (ncol(drybean))]
  Y_test <- drybean[-index, (ncol(drybean))]
