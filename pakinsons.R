...
parkinsons <- read.csv("C:/Users/Ray Chen/Desktop/data/pd_speech_features.csv",header = FALSE)
parkinsons.name <- colnames(parkinsons)
parkinsons.name[length(parkinsons.name)] <- "parkinsonstype"
colnames(parkinsons) <- parkinsons.name
parkinsons <- parkinsons[-1:-2,]

parkinsons$parkinsonstype <- as.factor(parkinsons$parkinsonstype)
for (i in 1:(ncol(parkinsons)-1)) {
  parkinsons[,i] <- as.numeric(parkinsons[,i])
  parkinsons[,i] <- scale(parkinsons[,i])
}
...
# PCA
pca_result <- prcomp(parkinsons[, 1:(ncol(parkinsons)-1)], center = TRUE, scale. = FALSE)
summary(pca_result)



cumulative_variance <- cumsum(summary(pca_result)$importance[2, ] * 100)

# 创建一个数据框用于绘图
cumulative_data <- data.frame(
  Principal_Component = factor(1:length(cumulative_variance)),
  Cumulative_Variance_Explained = cumulative_variance
)

# 使用ggplot2绘制累计方差解释比例的折线图
ggplot(cumulative_data, aes(x = Principal_Component, y = Cumulative_Variance_Explained)) +
...
pca_features <- as.data.frame(pca_result$x[,1:150])
nn <- 1e6
...
max_it <- 10
set.seed(123)
folds <- createFolds(parkinsons$parkinsonstype, k = max_it, list = TRUE, returnTrain = TRUE)
...
pb <- progress_bar$new(
  ...
  total = max_it, clear = FALSE, width = 60
)
for (m in 1:max_it) {
  pb$tick()
  #folds <- createFolds(parkinsons$parkinsonstype, k = max_it, list = TRUE, returnTrain = TRUE)
  index <- folds[[m]]
  X_train <- parkinsons[index, 1:(ncol(parkinsons)-1)]
  X_test <- parkinsons[-index, 1:(ncol(parkinsons)-1)]
  Y_train <- parkinsons[index, (ncol(parkinsons))]
  Y_test <- parkinsons[-index, (ncol(parkinsons))]
...
result[1,ncol(accuracy_matrix)] <- max(output[,ncol(accuracy_matrix)])
print(result)
