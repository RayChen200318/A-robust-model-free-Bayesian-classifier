...
car <- read.csv("C:/Users/Ray Chen/Desktop/data/car.data",header = FALSE)
car.name <- colnames(car)
car.name[length(car.name)] <- "cartype"
colnames(car) <- car.name
car$cartype <- as.factor(car$cartype)
cartype <- car$cartype
car <- car[, -ncol(car)]
dummies <- dummyVars(~ ., data = car)
car_encoded_dummyVars <- data.frame(predict(dummies, newdata = car))
car <- cbind(car_encoded_dummyVars,cartype)

for (i in 1:(ncol(car)-1)) {
  car[,i] <- as.numeric(car[,i])
}
...
# PCA
pca_result <- prcomp(car[, 1:(ncol(car)-1)], center = TRUE, scale. = FALSE)
summary(pca_result)
pca_features <- as.data.frame(pca_result$x[,1:15])
...
max_it <- 10
set.seed(123)
folds <- createFolds(car$cartype, k = max_it, list = TRUE, returnTrain = TRUE)
...
for (m in 1:max_it) {
  ...
  #folds <- createFolds(car$cartype, k = max_it, list = TRUE, returnTrain = TRUE)
  index <- folds[[m]]
  X_train <- car[index, 1:(ncol(car)-1)]
  X_test <- car[-index, 1:(ncol(car)-1)]
  Y_train <- car[index, (ncol(car))]
  Y_test <- car[-index, (ncol(car))]
